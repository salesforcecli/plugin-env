...
// TODO: test is waiting on delete sand box command to be available
  ...
  it('will create a sandbox, verify it can be opened, and then attempt to delete it', async () => {
    ...
    try {
      ...
      result = execCmd<SandboxProcessObject>(
        `env:create:sandbox -a mySandbox -s -p licenseType='Developer' -w 60 -o ${process.env.TESTKIT_HUB_USERNAME} --json`,
        { timeout: 3600000 }
      ).jsonOutput.result;
